...
function generateDays(date, nbDays, startTime, endTime, interval, randomSlotsToDelete = 0) {
  ...
  startingDay.setDate(date.getDate() + 1);
  for (let i = 1; i < nbDays; i += 1) {
    var slotsDate = new Date(startingDay);
    /**
    ...
    const slots = generateSlots(startDate, endDate, interval, randomSlotsToDelete);
    const meetingsDay = {
      date: slotsDate,
      slots,
    };
    ...
    // }
    slotsDate = new Date(startingDay.setDate(startingDay.getDate() + 1));
  }
  return days;
}
